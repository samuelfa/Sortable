...
async function dragByOffset(page, element, deltaX, deltaY, offsetX = 0, offsetY = 0) {
...
test.describe('Empty Insert', () => {
  ...
  test('Insert into empty list if within emptyInsertThreshold', async ({ page }) => {
    ...
    const threshold = await page.evaluate(() =>
      Sortable.get(document.getElementById('list2')).option('emptyInsertThreshold')
    );
  ...
  test('Do not insert into empty list if outside emptyInsertThreshold', async ({ page }) => {
    const list1 = page.locator('#list1');
    const list2 = page.locator('#list2');
